...
import { useAllFeaturedList } from '@/store/hook/useResourceList'
...
const ChristmasList: React.FC = () => {
  const videoRefs = useRef<{ [key: number]: Player | null }>({})
  const swiperRef = useRef<SwiperType>()
  const { getCurrentUid } = useTMAUtils()
  const currentUid = getCurrentUid()
  const { list: rawList, isLoading, page, hasMore } = useStore((state) => state.allFeaturedList)
  const setAllFeaturedPage = useStore((state) => state.setAllFeaturedPage)
