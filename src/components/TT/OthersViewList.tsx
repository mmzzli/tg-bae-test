...
const OthersViewList: React.FC = () => {
  const videoRefs = useRef<{ [key: number]: Player | null }>({})
  const swiperRef = useRef<SwiperType>()
  const { getCurrentUid } = useTMAUtils()
  const currentUid = getCurrentUid()
  const { list: rawList, isLoading, page, hasMore } = useStore((state) => state.othersViewList)
  const setOthersViewPage = useStore((state) => state.setOthersViewPage)
